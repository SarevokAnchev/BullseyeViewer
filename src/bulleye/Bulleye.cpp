...
Bulleye::Bulleye(BulleyeConfig cfg, const double *apex_, const double *axis_, double height_, const double *anterior_axis_)
{
    switch (cfg) {
        case BulleyeConfig::bulleye_16_segments:
            n_rings = 3;
            n_segments = {1, 4, 6, 6};
            break;
        case BulleyeConfig::bulleye_17_segments:
            n_rings = 3;
            n_segments = {4, 6, 6};
            break;
        case BulleyeConfig::bulleye_18_segments:
            n_rings = 3;
            n_segments = {6, 6, 6};
            break;
    }
    ...
    for (int i = 0; i < n_rings; i++) {
        ...
        for (int j = 0; j < n_segments[i]; j++) {
            segments.emplace_back(BulleyeSegment(
                ring_start_radius,
                ring_radius,
...
Bulleye::Bulleye(int n_rings_, const int *n_segments_, const double *apex_, const double *axis_,
...
Bulleye::Bulleye(BulleyeConfig cfg, const double *apex_, const double *base_, const double *front_)
{
    switch (cfg) {
        case BulleyeConfig::bulleye_16_segments:
            n_rings = 3;
...
const BulleyeSegment* Bulleye::is_inside(double x, double y, double z) const
{
    double radius, angle;
...
void Bulleye::polar_coordinates(double x, double y, double z, double& angle_, double& radius_) const
{
...
void Bulleye::add_scalars(const std::string& label_, std::vector<std::pair<double, double>> coords_, std::vector<double> values_)
{
...
void Bulleye::add_scalars(const std::string &label_, const std::vector<std::tuple<double, double, double>>& coords_,
...
void Bulleye::get_scalars(const std::string &label_, std::vector<std::pair<double, double>>& coords_,
                          std::vector<double>& values_)
{
    int idx = -1;
...
void Bulleye::get_scalars(int index, std::vector<std::pair<double, double>>& coords_,
                          std::vector<double>& values_)
{
...
bool Bulleye::scalars_exists(const std::string &label_) const
{
